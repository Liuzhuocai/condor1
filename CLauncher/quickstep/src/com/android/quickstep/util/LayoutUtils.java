...
public class LayoutUtils {
    ...
    public static void calculateLauncherTaskSize(Context context, DeviceProfile dp, Rect outRect) {
        ...
        float extraSpace = context.getResources().getDimensionPixelSize(R.dimen.quickstep_task_extra_space);
        if (!dp.isVerticalBarLayout()) {
            // Perry: Optimizing recents in multi window mode: start
            extraSpace += dp.allAppsSearchBarFieldHeightPx + dp.allAppsCellHeightPx
                + (dp.isMultiWindowMode ? Utils.dp2px(context, 4) : 0);
    ...
    public static void calculateTaskSize(Context context, DeviceProfile dp,
        ...
        int launcherVisibleHeight = dp.availableHeightPx;
        ...
        float y = insets.top + Math.max(topIconMargin,
                launcherVisibleHeight - extraVerticalSpace - outHeight);
        // Perry: Add clear all button to recents: end
