...
public abstract class RecentsView<T extends BaseActivity> extends PagedView implements Insettable, AllTaskClearClient.OnAllTaskClearListener {
    ...
    public static final FloatProperty<RecentsView> CLEAR_ALL_BUTTON_PROGRESS =
            new FloatProperty<RecentsView>("clearAllButtonProgress") {
                @Override
                public void setValue(RecentsView view, float value) {
                    view.setClearAllButtonProgress(value);
                }

                @Override
                public Float get(RecentsView view) {
                    return view.getClearAllButtonProgress();
                }
            };

    public static final FloatProperty<RecentsView> CLEAR_ALL_BUTTON_ALPHA =
            new FloatProperty<RecentsView>("clearAllButtonAlpha") {
                @Override
                public void setValue(RecentsView view, float value) {
                    if (view.mClearAllButton != null) {
                        view.mClearAllButton.setAlpha(value);
                    }
                }
                ...
                public Float get(RecentsView view) {
                    ...
                    return view.mClearAllButton.getAlpha();
                }
            };

    public void setClearAllButtonProgress(float progress) {
        if (mActivity instanceof Launcher && mClearAllButton != null) {
            Launcher launcher = (Launcher)mActivity;
            float transY = (progress - OVERVIEW.getVerticalProgress(launcher)) *
                    mActivity.getShiftRange();
            mClearAllButton.setTranslationY(transY);
        }
    }

    private float getClearAllButtonProgress() {
        if (!(mActivity instanceof Launcher) || mClearAllButton == null) {
            return 0;
        }

        Launcher launcher = (Launcher)mActivity;
        return mClearAllButton.getTranslationY() /
                mActivity.getShiftRange() + OVERVIEW.getVerticalProgress(launcher);
    }
    ...
    // Perry: Add clear all button to recents: start
    private final int mClearAllButtonId;
    ...
    // Perry: Optimizing recents UI: start
    private final int mClearAllButtonSpace;
    // Perry: Optimizing recents UI: end
    ...
    public RecentsView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs,
                R.styleable.RecentsView, defStyleAttr, 0);
        mClearAllButtonId = a.getResourceId(R.styleable.RecentsView_clearAllButton, -1);
        a.recycle();
        // Perry: Add clear all button to recents: end
        ...
        // Perry: Optimizing recents UI: start
        mClearAllButtonSpace = getResources()
                .getDimensionPixelSize(R.dimen.recents_clear_all_button_space);
        // Perry: Optimizing recents UI: end
    }
    ...
    public void initParentViews(View parent) {
        ...
        if (mClearAllButtonId > -1) {
            mClearAllButton = parent.findViewById(mClearAllButtonId);
            if (mClearAllButton instanceof AllTaskClearClient) {
                ((AllTaskClearClient) mClearAllButton).setOnAllTaskClearListener(this);
            }
            // Perry: First enter recents, all clear button not display: start
            mClearAllButton.setVisibility(getVisibility());
            mClearAllButton.setAlpha(getAlpha());
            // Perry: First enter recents, all clear button not display: end
        }
    }
    // Perry: Add clear all button to recents: end
    ...
    protected void onAttachedToWindow() {
        ...
        // Perry: Add clear all button to recents: start
        initParentViews((View)getParent());
        // Perry: Add clear all button to recents: end
        // Perry: Implement Lock/Unlock task function: start
        LockTaskManager.obtain().registerObserver(mActivity, ()-> {
            post(()-> {
    ...
    protected void onDetachedFromWindow() {
        ...
        // Perry: Implement Lock/Unlock task function: start
        LockTaskManager.obtain().unregisterObserver(mActivity);
        // Perry: Implement Lock/Unlock task function: end
    }
    ...
    public void onViewRemoved(View child) {
        ...
        if (mClearAllButton instanceof AllTaskClearClient) {
            mClearAllButton.post(()-> ((AllTaskClearClient)mClearAllButton).updateMemoryInfo());
        }
    ...
    public void setInsets(Rect insets) {
        ...
        if (mClearAllButton != null) {
            FrameLayout.LayoutParams lp = (FrameLayout.LayoutParams) mClearAllButton.getLayoutParams();
            // Perry: Optimizing recents UI: start
            lp.topMargin = mTempRect.bottom + mClearAllButtonSpace;
            // Perry: Optimizing recents UI: end
    ...
    public PendingAnimation createAllTasksDismissAnimation(long duration, Runnable completeRunnable) {
        if (FeatureFlags.IS_DOGFOOD_BUILD && mPendingAnimation != null) {
            throw new IllegalStateException("Another pending animation is still running");
        }
        AnimatorSet anim = new AnimatorSet();
        ...
        // Perry: Implement Lock/Unlock task function: start
        final ArrayList<TaskView> unLockTasks = new ArrayList<>();
        for (int i = 0; i < count; i++) {
            TaskView taskView = (TaskView) getPageAt(i);
            if (!taskView.isLocked()) {
                unLockTasks.add(taskView);
            }
        }

        unLockTasks.forEach((tv)-> addDismissedTaskAnimations(tv, anim, duration));
        // Perry: Implement Lock/Unlock task function: end
        mPendingAnimation = pendingAnimation;
        mPendingAnimation.addEndListener((onEndListener) -> {
            if (onEndListener.isSuccess) {
                // Perry: Implement Lock/Unlock task function: start
                unLockTasks.forEach((tv)-> {
                    removeTask(tv.getTask(), -1, onEndListener, false);
                    removeView(tv);
                });
                // Perry: Implement Lock/Unlock task function: end
                // Perry: Add clear all button to recents: start
    ...
    private void dismissAllTasks(View view) {
        runDismissAnimation(createAllTasksDismissAnimation(DISMISS_TASK_DURATION, this::onAllTasksRemoved));
    }

    // Perry: Add clear all button to recents: start
    @Override
    public void dismissAllTasks(Runnable completeRunnable) {
        runDismissAnimation(createAllTasksDismissAnimation(DISMISS_TASK_DURATION, completeRunnable));
    }
    // Perry: Add clear all button to recents: end
    ...
    public void setContentAlpha(float alpha) {
        ...
        // Perry: Add clear all button to recents: start
        int visibility = alpha > 0 ? VISIBLE : GONE;
        int alphaInt = Math.round(alpha * 255);
        mEmptyMessagePaint.setAlpha(alphaInt);
        mEmptyIcon.setAlpha(alphaInt);
        if (mClearAllButton != null) {
            mClearAllButton.setAlpha(mContentAlpha);
