...
public class TaskView extends FrameLayout implements TaskCallbacks, PageCallbacks {
    ...
    public void onTaskDataLoaded(Task task, ThumbnailData thumbnailData) {
        ...
        // Perry: Implement Lock/Unlock task function: start
        mIconView.setDrawable(getTaskIcon());
        // Perry: Implement Lock/Unlock task function: end
    ...
    public Drawable getTaskIcon() {
        return getTaskIcon(mTask);
    }

    public Drawable getTaskIcon(Task task) {
        if (task.key == null || task.getTopComponent() == null) {
            return task.icon;
        }
        LauncherAppState appState = LauncherAppState.getInstance(getContext());
        Task.TaskKey key = task.key;
        return appState.getIconCache().getIconFromCache(task.getTopComponent(),
                UserHandle.of(key.userId));
    }

    public void updateLockFlag() {
        mIconView.setLocked(isLocked());
    }

    public boolean isLocked() {
        return LockTaskManager.obtain().isTaskLocked(mTask.getTopComponent(),
