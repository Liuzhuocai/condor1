...
public class PortraitStatesTouchController extends AbstractStateChangeTouchController {
    ...
    protected boolean canInterceptTouch(MotionEvent ev) {
        ...
        if (mLauncher.isInState(ALL_APPS)) {
            ...
            View child = recentsView.getChildAt(recentsView.getNextPage());
            if (child instanceof TaskView && mLauncher.getDragLayer().
                    isEventOverView(child, ev)) {
                return false;
            }
    ...
    protected LauncherState getTargetState(LauncherState fromState, boolean isDragTowardPositive) {
        if (fromState == ALL_APPS && !isDragTowardPositive) {
            ...
            // Perry: Adjust recents UI: start
            return TouchInteractionService.isConnected() ? OVERVIEW :
                    DesktopModeHelper.isClassicMode() ? ALL_APPS : fromState;
            // Perry: Adjust recents UI: end
            // Perry: Add gesture for some states: start
        } else if (fromState == EDIT_MODE && isDragTowardPositive) {
            return TouchInteractionService.isConnected() ? OVERVIEW : fromState;
        }
    ...
    protected float initCurrentAnimation(@AnimationComponents int animComponents) {
        ...
        // Perry: Adjust recents UI: start
        if (!Utils.swipeDownToExitRecents() &&
                recentsView.shouldSwipeDownLaunchApp() && mFromState == OVERVIEW && mToState == NORMAL
                && taskView != null) {
