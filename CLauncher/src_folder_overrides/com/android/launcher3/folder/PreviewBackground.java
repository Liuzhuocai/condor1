...
public class PreviewBackground {
    ...
    //liuzuo:change foldericon bg:start
    private Bitmap folder_icon_bg;
    private int mCornerRadius;
    //liuzuo:change foldericon bg:end
    ...
    public void setup(Launcher launcher, View invalidateDelegate,
        ...
        mBgColor = Color.WHITE/* Themes.getAttrColor(launcher, android.R.attr.colorPrimary)*/;

        DeviceProfile grid = launcher.getDeviceProfile();
        //liuzuo:change folderIcon bg:end
        final int previewSize = grid.folderIconSizePx;
        final int previewPadding = grid.folderIconPreviewPadding;
        //liuzuo:change previewSize:begin

        //liuzuo:change previewSize:begin
        //this.previewSize = (previewSize - 2 * previewPadding);
        double sqrt = Math.sqrt(IconNormalizer.MAX_SQUARE_AREA_FACTOR / 1);
        this.previewSize = (int) ((previewSize - 2 * previewPadding)*sqrt);
        //liuzuo:change previewSize:end

        //liuzuo:offset bg:begin
        basePreviewOffsetX = (availableSpaceX - this.previewSize) / 2
                +1;
                //liuzuo:offset bg:end
        //liuzuo:change previewSize:begin
        // basePreviewOffsetY = topPadding + grid.folderIconOffsetYPx;
        basePreviewOffsetY = (int) (previewPadding + grid.folderBackgroundOffset + topPadding
                +previewSize*(1-sqrt)/2)+1;
        //liuzuo:change previewSize:end
        // Stroke width is 1dp
        ...
        float scale = 1f;
        if(mStrokeWidth<2){
            scale = mStrokeWidth/2f;
        }
        mCornerRadius = (int) (launcher.getResources().getDimensionPixelSize(R.dimen.folder_corner_radius)
                *scale);
        //liuzuo:offset bg:end
    ...
    public void drawBackgroundStroke(Canvas canvas) {
        mPaint.setColor(ColorUtils.setAlphaComponent(mBgColor, 0x4d));
    ...
    private void drawCircle(Canvas canvas,float deltaRadius) {
        //liuzuo:RoundrectPath:begin
       /* float radius = getScaledRadius();
        canvas.drawCircle(radius + getOffsetX(), radius + getOffsetY(),
                radius - deltaRadius, mPaint);*/
        if(folder_icon_bg == null){
            folder_icon_bg = ThemeUtils.getBitmapByName("folder_icon_bg.png");
        }
        if(folder_icon_bg!=null){
            float r = getScaledRadius();
            BitmapDrawable bitmapDrawable = new BitmapDrawable(folder_icon_bg);
            bitmapDrawable.setBounds((int)(getOffsetX()-deltaRadius),(int)(getOffsetY()-deltaRadius), (int)(getOffsetX()+r* 2+deltaRadius),(int)(getOffsetY()+r*2+deltaRadius));
            bitmapDrawable.draw(canvas);
            Log.d("theme1","drawBitmap");
//            canvas.drawBitmap(folder_icon_bg,null,new Rect(0,0,folder_icon_bg.getWidth(),folder_icon_bg.getHeight()),null);
        }else {
            Log.d("theme1","drawPath");
            canvas.drawPath(getRoundRectPath(deltaRadius),mPaint);
        }
        //liuzuo:RoundrectPath:end
    }

    public Path getClipPath() {
        //liuzuo:folderIcon change:begin
        float r = getScaledRadius();
        mPath.addRoundRect(getOffsetX(),getOffsetY(),
                getOffsetX()+r* 2,getOffsetY()+r*2,mCornerRadius,mCornerRadius,Path.Direction.CW);
        return mPath;
        //liuzuo:folderIcon change:end
    }
    ...
    // Perry: fix folder icon display error: start
    private void animateScale(float finalScale, float finalMultiplier,
                              final Runnable onStart, final Runnable onEnd) {
        animateScale(finalScale, finalMultiplier, onStart, onEnd, false);
    }

    private void animateScale(float finalScale, float finalMultiplier,
                              final Runnable onStart, final Runnable onEnd, boolean endImmediately) {
        ...
        mScaleAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                //liuzuo : change folder icon UI:start
                //mColorMultiplier = prog * bgMultiplier1 + (1 - prog) * bgMultiplier0;
                //liuzuo : change folder icon UI:end
                invalidate();
            }
        });
    ...
    // Perry: fix folder icon display error: end
    ...
    public void reset() {
        ...
        animateScale(1f, 1f, onStart, onEnd, true);
        // Perry: fix folder icon display error: end
    }
    ...
    //liuzuo:RoundrectPath:begin
    public Path getRoundRectPath(float deltaRadius) {
        mPath.reset();
        float r = getScaledRadius();
        mPath.addRoundRect(getOffsetX()-deltaRadius,getOffsetY()-deltaRadius,
                getOffsetX()+r* 2+deltaRadius,getOffsetY()+r*2+deltaRadius,mCornerRadius,mCornerRadius,Path.Direction.CW);
        return mPath;
    }
    //liuzuo:RoundrectPath:end
}
