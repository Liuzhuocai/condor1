...
public class SudokuFolderIconLayoutRule implements FolderIcon.PreviewLayoutRule {
    ...
    public void init(int availableSpace, float intrinsicIconSize, boolean rtl ,Resources r ) {
        ...
        Log.d("liuzuo95","SudokuFolderIconLayoutRule mThemeFolderBgScale="+mThemeFolderBgScale);
    }
...
//    }
    ...
    private float getPaddingX(int index) {
        int i = index%3;
        if(i == 0){
            return folderPaddingLeft*(mIsRtl?-mThemeFolderBgScale:mThemeFolderBgScale);
        }else if(i == 1){
            return 0;
        }else if(i == 2){
            return folderPaddingRight*(mIsRtl?-mThemeFolderBgScale:mThemeFolderBgScale);
        }
        return 0;
    }
    private float getPaddingY(int index) {
        int i = index/3;
        if(i == 0){
            return folderPaddingTop*mThemeFolderBgScale;
        }else if(i == 1){
            return 0;
        }else if(i == 2){
            return folderPaddingBottom*mThemeFolderBgScale;
        }
        return 0;
    }
}
