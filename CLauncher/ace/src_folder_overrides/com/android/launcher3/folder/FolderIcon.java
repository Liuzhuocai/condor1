...
public class FolderIcon extends FrameLayout implements FolderListener {
    ...
    public enum FolderIconMode {SudokuFolderIconLayoutRule,StackFolderIconLayoutRule,ClippedFolderIconLayoutRule,FourGongGeFolderIconLayoutRule}
    private FolderIconMode mFolderIconMode = CondorFeatureFlags.LAUNCHER3_LEGACY_FOLDER_ICON_RULE;
    public static  int NUM_ITEMS_IN_PREVIEW = SudokuFolderIconLayoutRule.MAX_NUM_ITEMS_IN_PREVIEW;
    ...
    private void init() {
        ...
        //M:liuzuo add for update foldericon begin
        ThemeConfigBean themeConfigBean = ThemeUtils.getThemeConfigBean();
        if(themeConfigBean!=null){
            int count = themeConfigBean.getCount();
            if(count==9){
                mFolderIconMode = FolderIconMode.SudokuFolderIconLayoutRule;
            }else if(count==4){
                mFolderIconMode = FolderIconMode.FourGongGeFolderIconLayoutRule;
            }
            Log.d("liuzuo99","init ThemeConfigBean="+themeConfigBean.toString());
        }
        switch (mFolderIconMode){
            ...
            case SudokuFolderIconLayoutRule:
                ...
                break;
            case FourGongGeFolderIconLayoutRule:
                mPreviewLayoutRule = new FourGongGeFolderIconLayoutRule();
                NUM_ITEMS_IN_PREVIEW = FourGongGeFolderIconLayoutRule.MAX_NUM_ITEMS_IN_PREVIEW;
                break;
        }
//        mPreviewLayoutRule = FeatureFlags.LAUNCHER3_LEGACY_FOLDER_ICON ?
//                new StackFolderIconLayoutRule() :
//                new ClippedFolderIconLayoutRule();
        //M:liuzuo add for update foldericon end
    ...
    private void onDrop(final ShortcutInfo item, DragView animateView, Rect finalRect,
        ...
        if (animateView != null) {
            ...
            int numItemsInPreview = Math.min(NUM_ITEMS_IN_PREVIEW, index + 1);
            boolean itemAdded = false;
            if (itemReturnedOnFailedDrop || index >= NUM_ITEMS_IN_PREVIEW
                    //liuzuo:change folder item anim:start
                    && mPreviewLayoutRule.hasEnterExitIndices()
                    //liuzuo:change folder item anim:end
    ...
    private float getLocalCenterForIndex(int index, int curNumItems, int[] center) {
        mTmpParams = mPreviewItemManager.computePreviewItemDrawingParams(
                Math.min(NUM_ITEMS_IN_PREVIEW, index), curNumItems, mTmpParams);
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (!mBackground.drawingDelegated()) {
            //liuzuo:change foldericon :begin
            //mBackground.drawBackgroundStroke(canvas);
            //liuzuo:change foldericon :end
        }
    ...
    public List<BubbleTextView> getPreviewItemsOnPage(int page) {
        ...
        List<BubbleTextView> itemsOnPage = mFolder.getFolderItems();
        ...
        for (int rank = 0; rank < numItems; ++rank) {
            ...
            if (itemsToDisplay.size() == NUM_ITEMS_IN_PREVIEW) {
                break;
            }
        }
    ...
    public boolean shouldTextBeVisible() {
        return mFolderName.shouldTextBeVisible();
    }

    public Animator createTextAlphaAnimator(boolean fadeIn) {
        return mFolderName.createTextAlphaAnimator(fadeIn);
    }
    ...
    public Rect getIconBounds() {
        ...
        int offsetY = mBackground.getOffsetY();
        int previewSize = (int) (mBackground.previewSize * mBackground.mScale);
        return new Rect(offsetX, offsetY, offsetX + previewSize, offsetY + previewSize);
    }
    // Perry: add extra system shortcuts: end
    //liuzuo:add for folder icon:start
    interface PreviewLayoutRule {
        PreviewItemDrawingParams computePreviewItemDrawingParams(int index, int curNumItems,
                                                                 PreviewItemDrawingParams params);

        void init(int availableSpace, float intrinsicIconSize, boolean rtl, Resources r);
