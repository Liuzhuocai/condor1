...
public class FolderAnimationManager {
    ...
    public AnimatorSet getAnimator() {
        ...
        int iconSize = itemsInPreview.isEmpty() ? mFolder.getIconSize() :
                itemsInPreview.get(0).getIconSize();
        ...
        if (Utilities.isRtl(mContext.getResources())) {
            previewItemOffsetX = (int) (mFolder.getDisplayWidth() * initialScale - initialSize - previewItemOffsetX);
        }

        Rect outRect = new Rect();
        mFolder.getHitRectRelativeToDragLayer(outRect);
        final int paddingOffsetX = (int) ((mFolder.getPaddingLeft() + mContent.getPaddingLeft())
                * initialScale);
        final int paddingOffsetY = (int) ((mFolder.getPaddingTop() + mContent.getPaddingTop() - mFolder.getScrollY())
                * initialScale);
        ...
        final float xDistance = initialX - grid.folderPagePaddingLeft;
        final float yDistance = initialY - grid.folderPagePaddingTop;

        // Set up the Folder background.
        final int finalColor = mContext.getColor(R.color.spring_loaded_panel_color);
        final int initialColor = mPreviewBackground.getBgColor();
//        mFolderBackground.setColor(mIsOpening ? initialColor : finalColor);
//        mFolderBackground.setAlpha(0xFF);
        ...
        Rect endRect = mFolder.getBackground().getBounds();
        float initialRadius =
                initialSize / initialScale / 2f;
        float finalRadius = mContent.getResources().getDimensionPixelSize(R.dimen.folder_corner_radius);
        ...
        PropertyResetListener colorResetListener =
                new PropertyResetListener<>(BubbleTextView.TEXT_ALPHA_PROPERTY, 1f);
        for (BubbleTextView icon : mFolder.getFolderItems()) {
            if (mIsOpening) {
        ...
        //liuzuo : add alpha animator :start
        play(a, getAnimator(mFolder, View.ALPHA, 0,1f));
        //liuzuo : add alpha animator :end
        ...
        mFolder.getContainer().getLabel().setAlpha(mIsOpening ? 1 : 0);
        play(a, getAnimator(0, 1, va-> {
            mFolder.getContainer().setDarkenProgress((float) va.getAnimatedValue());
            mLauncher.setWorkspaceAlpha(1-(float) va.getAnimatedValue(), false);
        }));
        ...
        a.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationStart(Animator animation) {
                ...
                FolderPagesContainer container = mFolder.getContainer();
                container.setNearestFoldersVisible(false);
                container.setClipChildren(false);
                FolderViewPager pager = container.getPager();
                if (pager != null) {
                    pager.setClipChildren(mFolder.getItemCount() > FolderIcon.NUM_ITEMS_IN_PREVIEW);
                }
            }
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                FolderPagesContainer container = mFolder.getContainer();
                container.setNearestFoldersVisible(true);
    ...
    private void addPreviewItemAnimators(AnimatorSet animatorSet, final float folderScale,
        ...
        FolderIcon.PreviewLayoutRule rule = mFolderIcon.getLayoutRule();
        final List<BubbleTextView> items = mFolder.getFolderItems();
        final int numItemsInPreview = items.size();

        TimeInterpolator previewItemInterpolator = getPreviewItemInterpolator();
        final boolean isStackFolderIcon = rule instanceof StackFolderIconLayoutRule;
        ShortcutAndWidgetContainer cwc = mContent.getShortcutsAndWidgets();
        for (int i = 0; i < mFolder.getItemCount(); ++i) {
            final BubbleTextView btv = items.get(i);
            ...
            float previewScale = isStackFolderIcon ?
                    ((StackFolderIconLayoutRule) rule).totalScaleForItem(i) : rule.scaleForItem(0,numItemsInPreview);
            float previewSize = rule.getIconSize() * previewScale;
            float iconScale = previewSize / items.get(i).getIconSize();
            ...
            final boolean overMaxNumItemsInPreview = i >= FolderIcon.NUM_ITEMS_IN_PREVIEW;
            Animator alphaAnimator = null;
            if (overMaxNumItemsInPreview) {
                alphaAnimator = getAnimator(btv, View.ALPHA, 0, 1);
    ...
    private Animator getAnimator(float v1, float v2, ValueAnimator.AnimatorUpdateListener listener) {
        ValueAnimator va = mIsOpening
                ? ValueAnimator.ofFloat(v1, v2)
                : ValueAnimator.ofFloat(v2, v1);
