...
public class CaretDrawable extends Drawable {
    ...
    public CaretDrawable(Context context, boolean supportDarkTheme) {
        final Resources res = context.getResources();

        final int strokeWidth = res.getDimensionPixelSize(R.dimen.caret_stroke_width);
        final int shadowSpread = res.getDimensionPixelSize(R.dimen.caret_shadow_spread);

        if (supportDarkTheme) {
            boolean darkText = WallpaperColorInfo.getInstance(context).supportsDarkText();
            if (darkText) {
                mCaretPaint.setColor(Color.BLACK);
            } else {
                mCaretPaint.setColor(Color.WHITE);
            }
        } else {
            mCaretPaint.setColor(Color.WHITE);
        }
        ...
        mCaretSizePx = res.getDimensionPixelSize(R.dimen.caret_size);
    }

    public void setCaretColor(int color, int shadowAlpha) {
