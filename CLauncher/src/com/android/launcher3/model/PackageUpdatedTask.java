...
public class PackageUpdatedTask extends BaseModelUpdateTask {
    ...
    public void execute(LauncherAppState app, BgDataModel dataModel, AllAppsList appsList) {
            ...
            case OP_ADD: {
                for (int i = 0; i < N; i++) {
                    ...
                    if (DesktopModeHelper.isClassicMode() &&
                            !Utilities.ATLEAST_OREO && !Process.myUserHandle().equals(mUser)) {
        ...
        // Perry: desktop switch function: start
        final ArrayList<ItemInfo> addedApps = new ArrayList<>();
        addedOrModified.addAll(appsList.added);
        addedApps.addAll(appsList.added);
        ...
        addedOrModified.forEach(info-> {
            if (info.isFrozen()) {
                if (info.updateFrozenFlags()) {
                    addedApps.add(info);
                }
            } else {
                info.updateFrozenFlags();
            }
        });
        // Perry: Implement frozen apps: end

        if (DesktopModeHelper.isDefaultMode()) {
            mModel.addAndBindItemsToWorkspace(addedApps);
        }
        // Perry: desktop switch function: end
        ...
        if (mOp == OP_REMOVE) {
            ...
            // Perry: Implement frozen apps: start
            removedComponents.addAll(addedOrModified.stream().filter(AppInfo::isFrozen).map(info->info.componentName).
