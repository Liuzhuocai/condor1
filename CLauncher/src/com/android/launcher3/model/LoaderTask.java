...
public class LoaderTask implements Runnable {
    ...
    private void loadWorkspace() {
        ...
        synchronized (mBgDataModel) {
            ...
            if (DesktopModeHelper.isDefaultMode()) {
                StartPageHelper.loadStartPage(mBgDataModel.workspaceScreens.size());
            }
    ...
    private void loadAllApps() {
        ...
        final List<Pair<ItemInfo, Object>> addedToWorkspace = new ArrayList<>();

        final HashSet<ComponentKey> workspaceItems = new HashSet<>();
        synchronized (mBgDataModel) {
            for (ItemInfo info : mBgDataModel.itemsIdMap) {
                if (info.itemType == LauncherSettings.Favorites.ITEM_TYPE_APPLICATION) {
                    workspaceItems.add(ComponentKey.from(info));
                }
            }
        }
        ...
        final Context context = mApp.getContext();
        for (ActivityInfo app: FrozenAppsManager.obtain().getFrozenActivityList()) {
            mBgAllAppsList.add(new AppInfo(context, app, Process.myUserHandle()));
        }
        ...
        for (UserHandle user : profiles) {
            ...
            for (int i = 0; i < apps.size(); i++) {
                ...
                AppInfo info = new AppInfo(app, user, quietMode);
                // This builds the icon bitmaps.
                mBgAllAppsList.add(info, app);

                if (DesktopModeHelper.isDefaultMode()) {
                    if (!workspaceItems.contains(ComponentKey.from(info))) {
                        addedToWorkspace.add(Pair.create(info, app));
                    }
                }
            }
        }
        ...
        if (DesktopModeHelper.isDefaultMode()) {
            if (!addedToWorkspace.isEmpty()) {
                mApp.getModel().enqueueModelUpdateTask(new AddWorkspaceItemsTask(addedToWorkspace) {
                    @Override
                    protected boolean shouldIgnoringTask() {
                        return false;
                    }
    ...
    private void dumpItemList(List<Pair<ItemInfo, Object>> addedToWorkspace) {
        Logger.d(TAG, "==========================================================");
        Logger.d(TAG, "add other workspace items size: " + addedToWorkspace.size());
        for (Pair<ItemInfo, Object> entry : addedToWorkspace) {
            ItemInfo info = entry.first;
            Logger.d(TAG, "application: " + info.title + " (" + info.getTargetComponent() + ")");
        }
        Logger.d(TAG, "==========================================================");
    }
}
