...
public class AllAppsList {
    ...
    public void add(AppInfo info) {
        ...
        mIconCache.getTitleAndIcon(info, true /* useLowResIcon */);
    ...
    public void updatePackage(Context context, String packageName, UserHandle user) {
        final LauncherAppsCompat launcherApps = LauncherAppsCompat.getInstance(context);
        // Perry: Implement frozen apps: start
        if (updateFrozenPackage(context, packageName, user)) {
            return;
        }
        // Perry: Implement frozen apps: end
    ...
    // Perry: Implement frozen apps: start
    private boolean updateFrozenPackage(Context context, String packageName, UserHandle user) {
        final List<ActivityInfo> matches = FrozenAppsManager.obtain().getFrozenActivity(packageName);
        if (matches.size() > 0) {
            for (final ActivityInfo info : matches) {
                AppInfo applicationInfo = findAppInfo(new ComponentName(info.packageName, info.name),
                        user);
                if (applicationInfo == null) {
                    add(new AppInfo(context, info, user));
                } else {
                    mIconCache.getTitleAndIcon(applicationInfo, true /* useLowResIcon */);
                    modified.add(applicationInfo);
                }
            }
            return true;
