...
public class Workspace extends PagedView<WorkspacePageIndicator>
    ...
    public Workspace(Context context, AttributeSet attrs, int defStyle) {
        ...
        super.setTransformer(EFFECT_SWITCHER.get().getTransformer());
    ...
    // Perry: Click to exit edit mode: start
    @Override
    public void initParentViews(View parent) {
        ...
        setOnClickListener((v)-> {
            ...
            EditModePanel panel = mLauncher.getEditModePanel();
            if (panel != null && mLauncher.isInEditMode()) {
    ...
    // Perry: Click to exit edit mode: end
    ...
    public void stripEmptyScreens() {
        ...
        for (Long id: removeScreens) {
            ...
            if (getChildCount() > minScreens) {
                ...
                final int pageIndex = indexOfChild(cl);
                // Perry: Resolve bug 3:start
                if (pageIndex <= currentPage) {
                    pageShift++;
                }
                // Perry: Resolve bug 3:end
                ...
                if (DesktopModeHelper.isDefaultMode()) {
                    StartPageHelper.deletePage(pageIndex);
                }
        ...
        if (pageShift >= 0) {
            ...
            //liuzuo: bug: Alpha of CellLayout is equal to 0 when the number of pages is one:begin
            if(getPageCount() == 1){
                try {
                    final CellLayout cl = (CellLayout) getChildAt(0);
                    if(cl.getAlpha()==0f)
                        cl.setAlpha(1f);
                }catch (Exception  e){
            ...
            //liuzuo: bug: Alpha of CellLayout is equal to 0 when the number of pages is one:end
        }
    }
        ...
        if (!workspaceInModalState() && !mIsSwitchingState && !mDragController.isDragging()) {
            ...
            if (isLoopScroll()) {
                if (isXBeforeFirstPage()) {
                    updatePageAlphaValue((CellLayout) getChildAt(0), 0, screenCenter);
                    updatePageAlphaValue((CellLayout) getChildAt(childCount - 1), -2, screenCenter);
                } else if (isXAfterLastPage()) {
                    updatePageAlphaValue((CellLayout) getChildAt(0), childCount, screenCenter);
                    updatePageAlphaValue((CellLayout) getChildAt(childCount - 1), childCount-1, screenCenter);
                } else {
                    for (int i = 0; i < childCount; i++) {
                        CellLayout child = (CellLayout) getChildAt(i);
                        updatePageAlphaValue(child, i, screenCenter);
                    }
                }
            } else {
                for (int i = 0; i < childCount; i++) {
    ...
    // Perry: Cyclic sliding of desktop pages: start
    private void updatePageAlphaValue(CellLayout child, int page, int screenCenter) {
        if (child != null) {
            float scrollProgress = getScrollProgress(screenCenter, child, page);
    ...
    private void updateChildrenLayersEnabled() {
        // Perry: Optimizing sliding speed: start
        if (cannotTransform()) {
        ...
        // Perry: Optimizing sliding speed: end
    }
    ...
    public DragView beginDragShared(View child, DragSource source, ItemInfo dragObject,
        ...
        showPopupContainer(child, dragOptions);
        // Perry: add extra system shortcuts: end

        // Perry: desktop switch function: start
        if (DesktopModeHelper.isDefaultMode() &&
                source instanceof AllAppsContainerView) {
            Log.d(TAG, "If in default desktop mode, don't drag in all apps view");
            return null;
        }
        // Perry: desktop switch function: end
    ...
    public void showPopupContainer(View child, DragOptions options) {
        if ((CondorFeatureFlags.SUPPORT_EXTRA_SYSTEM_SHORTCUTS ||
                child instanceof BubbleTextView) && !options.isAccessibleDrag) {
            PopupContainerWithArrow popupContainer = PopupContainerWithArrow
                    .showForIcon(child);
            if (popupContainer != null) {
                options.preDragCondition = popupContainer.createPreDragCondition();
    ...
    public void onDrop(final DragObject d, DragOptions options) {
        ...
        if (d.dragSource != this || mDragInfo == null) {
            ...
            if (dropTargetLayout != null && !d.cancelled) {
                ...
                if (createUserFolderIfNecessary(cell, container,
                        ...
                        addToExistingFolderIfNecessary(cell, dropTargetLayout, mTargetCell,
                                distance, d, false)) {
                    // Perry: Add edit mode panel: start
                    mLauncher.getStateManager().goToStateFrom(NORMAL, SPRING_LOADED, SPRING_LOADED_EXIT_DELAY);
                    // Perry: Add edit mode panel: end
                    return;
                }
                    ...
                    if (container != LauncherSettings.Favorites.CONTAINER_HOTSEAT &&
                        ...
                        if (pInfo != null && pInfo.resizeMode != AppWidgetProviderInfo.RESIZE_NONE
                            ...
                            onCompleteRunnable = new Runnable() {
                                public void run() {
									// Perry: Sometimes the resize frame would not show: start
                                    post(()-> {
                                        if (!isPageInTransition()) {
                                            AppWidgetResizeFrame.showForWidget(hostView, cellLayout);
                                        }
                                    });
									// Perry: Sometimes the resize frame would not show: end
                                }
                            };
                        }
                    }
            ...
            mLauncher.getHotseat().arrangeContent();
            // Perry: Hotseat reorder function: end

            // Perry: Add edit mode panel: start
            mLauncher.getStateManager().goToStateFrom(
                    NORMAL, SPRING_LOADED, SPRING_LOADED_EXIT_DELAY, onCompleteRunnable);
            // Perry: Add edit mode panel: end
        }
   ...
   void mapPointFromSelfToHotseatLayout(Hotseat hotseat, float[] xy) {
    ...
    public void onDragOver(DragObject d) {
        ...
        // Perry: Hotseat reorder function: start
        final boolean dropLayoutChanged = setDropLayoutForDragObject(d, mDragViewVisualCenter[0], mDragViewVisualCenter[1]);
        if (dropLayoutChanged) {
        ...
        if (mDragTargetLayout != null) {
            // Perry: Hotseat reorder function: start
            boolean targetLayoutChanged = d.dragSource != this ||
                    mDragInfo != null && mDragInfo.cell.getParent().getParent() != mDragTargetLayout;
            if (dropLayoutChanged) {
                if (mLauncher.getDeviceProfile().isVerticalBarLayout()) {
                    if (targetLayoutChanged && mDragTargetLayout instanceof HotseatCellLayout
                            && mDragTargetLayout.getItemsCount() >= mDragTargetLayout.getCountY()) {
                        mDragTargetLayout.setGridSize(1, mDragTargetLayout.getCountY() + 1);
                    } else {
                        mLauncher.getHotseat().arrangeContent();
                    }
                } else {
                    if (targetLayoutChanged && mDragTargetLayout instanceof HotseatCellLayout
                            && mDragTargetLayout.getItemsCount() >= mDragTargetLayout.getCountX()) {
                        mDragTargetLayout.setGridSize(mDragTargetLayout.getCountX() + 1, 1);
                    } else {
                        mLauncher.getHotseat().arrangeContent();
                    }
                }
            ...
            if (!nearestDropOccupied) {
                ...
                mReorderAlarm.setAlarm(mDragTargetLayout.getReorderTimeout());
            }
    ...
    private void manageFolderFeedback(CellLayout targetLayout,
        ...
        // Perry: Hotseat reorder function: start
        boolean targetLayoutChanged = dragObject.dragSource != this ||
                mDragInfo != null && mDragInfo.cell.getParent().getParent() != targetLayout;
        if (distance > mMaxDistanceForFolderCreation) {
            if (targetLayout instanceof HotseatCellLayout) {
                if (mDragMode == DRAG_MODE_ADD_TO_FOLDER || mDragMode == DRAG_MODE_CREATE_FOLDER) {
                    setDragMode(DRAG_MODE_NONE);
                    if (mLauncher.getDeviceProfile().isVerticalBarLayout()) {
                        if (targetLayoutChanged && mDragTargetLayout.getItemsCount() >= mDragTargetLayout.getCountY()) {
                            mDragTargetLayout.setGridSize(1, mDragTargetLayout.getCountY() + 1);
                        }
                    } else {
                        if (targetLayoutChanged && mDragTargetLayout.getItemsCount() >= mDragTargetLayout.getCountX()) {
                            mDragTargetLayout.setGridSize(mDragTargetLayout.getCountX() + 1, 1);
                        }
                    }
                }
    ...
    class FolderCreationAlarmListener implements OnAlarmListener {
        ...
        public FolderCreationAlarmListener(CellLayout layout, int cellX, int cellY) {
            ...
            // Perry: Hotseat reorder function: start
            bg.setup(mLauncher, null,
                    layout instanceof HotseatCellLayout ? ((HotseatCellLayout) layout).getRealCellSize()[0]:
    ...
    public BubbleTextView findShortcutByAppInfo(AppInfo app) {
        ...
        for (ShortcutAndWidgetContainer layout: childrenLayouts) {
            BubbleTextView shortcut = layout.findShortcutByAppInfo(app);
            if (shortcut != null) {
                return shortcut;
            }
        }
    ...
    public FolderIcon findFolderIconById(long id) {
        ArrayList<ShortcutAndWidgetContainer> childrenLayouts = getAllShortcutAndWidgetContainers();
        for (ShortcutAndWidgetContainer layout: childrenLayouts) {
            FolderIcon folderIcon = layout.findFolderIconById(id);
            if (folderIcon != null) {
                return folderIcon;
            }
        }
    ...
    public boolean location(final BubbleTextView app) {
        final ShortcutInfo info = (ShortcutInfo) app.getTag();
        ...
        if (info.container == LauncherSettings.Favorites.CONTAINER_DESKTOP
            ...
            if (getPageIndexForScreenId(info.screenId) == getCurrentPage() || info.container ==
                    LauncherSettings.Favorites.CONTAINER_HOTSEAT) {
                LocateAnimationHelper.startLocateAnimation(app);
            } else {
                setDelayedSnapToPageRunnable(()-> LocateAnimationHelper.startLocateAnimation(app));
                snapToPage(getPageIndexForScreenId(info.screenId));
            }
        } else {
            final FolderIcon folderIcon = findFolderIconById(info.container);
            if (folderIcon == null) {
                return false;
            }
            ...
            if (getPageIndexForScreenId(folderInfo.screenId) == getCurrentPage()
                    || folderInfo.container == LauncherSettings.Favorites.CONTAINER_HOTSEAT) {
                startFolderLocationAnimation(folder, app);
            } else {
                setDelayedSnapToPageRunnable(()-> startFolderLocationAnimation(folder, app));
    ...
    private void startFolderLocationAnimation(final Folder folder, final BubbleTextView app) {
        LocateAnimationHelper.startLocateAnimation(folder.getFolderIcon(), ()-> folder.location(app));
    }
    // Perry: locate application function: end

    // Perry: Cyclic sliding of desktop pages: start
    @Override
    protected boolean isCycleSide() {
        if (getPageCount() < 2) {
            return false;
        }
        ...
        return SettingsPersistence.DESKTOP_LOOP_OPEN.value();
    }
    // Perry: Cyclic sliding of desktop pages: end
    ...
    public Rect getScaledHitRect(Rect rect) {
        if (rect == null) {
            rect = new Rect();
        }
        if (mLauncher.isInState(EDIT_MODE)) {
            View child = getChildAt(0);
            rect.set(0, child.getTop(), getMeasuredWidth(),
                    child.getTop() + child.getHeight());
        } else {
            getHitRectRelativeToDragLayer(rect);
        }
    ...
    public void setTransformer(BaseTransformer transformer) {
        ...
        int toPageIndex = getCurrentPage() + 1;
        if (toPageIndex >= getChildCount()) {
            toPageIndex = getChildCount() - 2;
        }
        setDelayedSnapToPageRunnable(()-> {
            if (!mLauncher.isInState(EDIT_MODE)) {
                return;
            }
        ...
        snapToPage(toPageIndex);
    }
    // Perry: Implement sliding effect function: end
    //lizuo add for add icon begin
    public Folder getFolderForTag(FolderInfo info) {
        return (Folder) getFirstMatch(new ItemOperator() {

            @Override
            public boolean evaluate(ItemInfo info, View view) {
                return (view instanceof Folder) && (((Folder) view).getInfo() == info)
                        ;
            }
    ...
    public boolean workspaceInModalStateForTouchIntercetp() {
        return !mLauncher.isInState(NORMAL)&&!mLauncher.isInState(EDIT_MODE)&&!mLauncher.isInState(IMPORT_MODE);
    }
    //lizuo add for add icon end
    ...
    public void setWorkspaceAlpha(float alpha, boolean forAllPages) {
        ...
        if (currentChild instanceof CellLayout) {
            currentChild.setAlpha(alpha);
        }

        setHotseatAlpha(alpha);
    ...
    private void setHotseatAlpha(float alpha) {
...
// liuzuo: Add  for ace folder: end
    ...
    public ArrayList<View> moveScreenItemsByMatcher(long screenId, LongSparseArray<ItemInfo> movedItems) {
        final CellLayout layoutParent = getScreenWithId(screenId);
        final ItemInfoMatcher matcher = ItemInfoMatcher.ofItemIds(movedItems);
        final ArrayList<View> movedViews = new ArrayList<>();
        if (layoutParent != null) {
            ...
            ArrayList<ItemInfo> itemsToMove = new ArrayList<>(matcher.filterItemInfos(items));
            itemsToMove.sort((o1, o2) -> {
            ...
            for (ItemInfo itemToMove : itemsToMove) {
                View child = idToViewMap.get(itemToMove.id);
                ItemInfo item = movedItems.get(itemToMove.id);
                if (item != null && child != null) {
                    layoutParent.removeView(child);
                    ItemInfo info = (ItemInfo) child.getTag();
                    info.cellX = item.cellX;
                    info.cellY = item.cellY;
                    addInScreen(child, info);
                    // update the item's position after drop
                    CellLayout.LayoutParams lp = (CellLayout.LayoutParams) child.getLayoutParams();
                    lp.cellX = lp.tmpCellX = item.cellX;
                    lp.cellY = lp.tmpCellY = item.cellY;
                    lp.cellHSpan = info.spanX;
                    lp.cellVSpan = info.spanY;
                    lp.isLockedToGrid = true;
                    layoutParent.onDropChild(child);
