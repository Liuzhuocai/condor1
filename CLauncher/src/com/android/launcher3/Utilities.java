...
public final class Utilities {
    ...
    public static int calculateTextHeight(float textSizePx) {
        ...
        return (int) Math.ceil(fm.bottom - fm.top)
                //liuzuo:change calculateTextHeight:begin
                *2;
                //liuzuo:change calculateTextHeight:end
    }
    ...
    public static int weakenColor(int color) {
        return (((int)(Color.alpha(color) * 0.5f)) << 24) + (color & 0x00FFFFFF);
    }
    // Perry: desktop switch function: end

    //liuzuo: isNycOrAbove:begin
    public static boolean isNycOrAbove() {
        return Build.VERSION.SDK_INT >= Build.VERSION_CODES.N;
    }

    public static boolean supportLegacyFolderIcon() {
        return false;
    }

    public static int getBitmapAlphaOffset(Bitmap bitmap, Rect padding, float alpha) {
        if (!bitmap.hasAlpha()) return 0;
        int halfWidth  = bitmap.getWidth() / 2;
        int halfHeight = bitmap.getHeight() / 2;

        int left = 0, top = 0, right = 0, bottom = 0, round = 0;
        for (int x = 0; x < halfWidth; x++) {
            if (Color.alpha(bitmap.getPixel(x, halfHeight)) > alpha) {
                break;
            }
            left++;
        }

        for (int x = bitmap.getWidth()-1; x > halfWidth; x--) {
            if (Color.alpha(bitmap.getPixel(x, halfHeight)) > alpha) {
                break;
            }
            right++;
        ...
        for (int y = 0; y < halfHeight; y++) {
            if (Color.alpha(bitmap.getPixel(halfWidth, y)) > alpha) {
                break;
            }
            top++;
        }

        for (int y = bitmap.getHeight()-1; y > halfHeight; y--) {
            if (Color.alpha(bitmap.getPixel(halfWidth, y)) > alpha) {
                break;
            }
            bottom++;
        ...
        for (int x = left; x < halfWidth;) {
            if (Color.alpha(bitmap.getPixel(x++, top)) > alpha) {
                break;
            }
            round++;
        }
    ...
    public static boolean isPermissionGranted(Context mContext, String permission) {
