...
public class WidgetPreviewLoader {
    ...
    public Bitmap generateWidgetPreview(BaseActivity launcher, LauncherAppWidgetProviderInfo info,
        ...
        if (scale != 1f) {
            ...
            //liuzuo:chang widget UI:begin
            int appIconSize = launcher.getDeviceProfile().iconSizePx;
            if(!widgetPreviewExists && appIconSize>previewHeight){
                previewHeight = appIconSize;
            }
            //liuzuo:chang widget UI:end
        }
    ...
    private Bitmap generateShortcutPreview(BaseActivity launcher, ShortcutConfigActivityInfo info,
        ...
        Bitmap icon = li.createScaledBitmapWithoutShadow(
                mutateOnMainThread(info.getFullResIcon(mIconCache)), 0,mContext);
        li.recycle();
    ...
    public void flush() {
        ...
        try {
            ...
            Log.w(TAG, "Clear cache database failed.", e);
        }
    }
