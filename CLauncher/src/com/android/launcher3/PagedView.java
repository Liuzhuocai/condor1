...
public abstract class PagedView<T extends View & PageIndicator> extends ViewGroup {
    ...
    private static final int OVER_FIRST_PAGE_INDEX = -2;
    ...
    // Perry: Cyclic sliding of desktop pages: start
	private final int[] mPageScrollsForCircleSlide = new int[] { 0, 0 };
    ...
    public int getNextPage() {
        ...
        if (mNextPage == getChildCount()) {
            page = 0;
        } else if (mNextPage == OVER_FIRST_PAGE_INDEX) {
    ...
    // Perry: Cyclic sliding of desktop pages: start
    private int validateNewPageWithCycle(int newPage) {
    ...
    public void scrollTo(int x, int y) {
        ...
        // Perry: Cyclic sliding of desktop pages: start
        if (hasOverlay() && isXBeforeFirstPage) {
            mLoopScroll = false;
        } else {
            mLoopScroll = isCycleSide();
        }
    ...
    protected boolean computeScrollHelper(boolean shouldInvalidate) {
        if (mScroller.computeScrollOffset()) {
            ...
            int prevPage = mCurrentPage;
            // Perry: Cyclic sliding of desktop pages: start
            if ((mPageScrolls != null) && shouldValidateForCycle()) {
    ...
    // Perry: Cyclic sliding of desktop pages: start
    private boolean shouldValidateForCycle() {
        return isLoopScroll() &&
                (mNextPage == OVER_FIRST_PAGE_INDEX || mNextPage == getPageCount());
    }
    // Perry: Cyclic sliding of desktop pages: end
    ...
    protected boolean getPageScrolls(int[] outPageScrolls, boolean layoutChildren,
        ...
        // Perry: Cyclic sliding of desktop pages: start
        final int endIndex = mIsRtl ? 0 : childCount - 1;
        final int endCount = mIsRtl ? -1: childCount;
        ...
        final int firstChildLeft = scrollOffsetLeft + offsetForPageScrolls();
        boolean pageScrollChanged = false;

        for (int i = startIndex, childLeft = firstChildLeft;
                i != endCount;
                i += delta) {
        ...
        mPageScrollsForCircleSlide[0] = firstChildLeft - (mPageSpacing + getPageAt(endIndex).getMeasuredWidth() + getChildGap()) - scrollOffsetLeft;
        mPageScrollsForCircleSlide[1] = outPageScrolls[endIndex] + mPageSpacing + getPageAt(startIndex).getMeasuredWidth() + getChildGap();
        // Perry: Cyclic sliding of desktop pages: end
    ...
    public int getScrollForPage(int index) {
        ...
        if (isLoopScroll()) {
            if (index == -2) {
                return mIsRtl ? mPageScrollsForCircleSlide[1] : mPageScrollsForCircleSlide[0];
            } else if (index == getChildCount()){
                return mIsRtl ? mPageScrollsForCircleSlide[0] : mPageScrollsForCircleSlide[1];
            }
        }
        // Perry: Cyclic sliding of desktop pages: end
            ...
            if (mTouchState == TOUCH_STATE_SCROLLING) {
                ...
                if (!mFreeScroll) {
                    ...
                    if (((isSignificantMove && !isDeltaXLeft && !isFling) ||
                            (isFling && !isVelocityXLeft)) && (isLoopScroll() ? (mCurrentPage >= 0) : (mCurrentPage > 0))) {
                        ...
                        snapToPageWithVelocity(finalPage, velocityX);
                            ...
                            (isLoopScroll() ? (mCurrentPage < getChildCount()) : (mCurrentPage < getChildCount() - 1))) {
                        finalPage = returnToOriginalPage ? mCurrentPage : mCurrentPage + 1;
                        snapToPageWithVelocity(finalPage, velocityX);
                    } else {
                        snapToDestination();
    ...
    protected boolean snapToPageWithVelocity(int whichPage, int velocity) {
        // Perry: Cyclic sliding of desktop pages: start
        if(isLoopScroll()){
            whichPage = validateNewPageWithCycle(whichPage);
        }else{
    ...
    public int[] getVisibleChildrenRange() {
        ...
        // Perry: Optimizing page sliding effect: start
        if (leftChild == childCount-1 && rightChild == childCount-1) {
            rightChild = 0;
        } else if (leftChild == 0 && rightChild == 0) {
            leftChild  = childCount-1;
        }
        // Perry: Optimizing page sliding effect: end
    ...
    // Perry: locate application function: start
    public void setDelayedSnapToPageRunnable(Runnable r) {
    ...
    protected void dispatchDraw(Canvas canvas) {
        if (isLoopScroll()) {
            boolean isXBeforeFirstPage = isXBeforeFirstPage();
            boolean isXAfterLastPage = isXAfterLastPage();
            if (isXBeforeFirstPage || isXAfterLastPage) {
                ...
                int offset = mIsRtl ? -mPageScrollsForCircleSlide[1] : mPageScrollsForCircleSlide[1];
                if (isXBeforeFirstPage) {
    ...
    protected final boolean isLoopScroll() {
        return mLoopScroll;
    }

    protected final boolean isXBeforeFirstPage() {
        return mIsRtl ? (mOverScrollX > mMaxScrollX) : (mOverScrollX < 0);
    }

    protected final boolean isXAfterLastPage() {
        return mIsRtl ? (mOverScrollX < 0) : (mOverScrollX > mMaxScrollX);
    }
    // Perry: Cyclic sliding of desktop pages: end
    ...
    protected boolean cannotTransform() {
        return mTransformer == null || getChildCount() <= 1;
    }

    private float computeScrolledFactor(int index, int width) {
        return (getScrollForPage(index) - getScrollX()) / (float)width;
    }
    ...
    private void transformPage() {
        ...
        final int childCount = getChildCount();
        // Perry: fix slide page exception: start
        final int width = getScrollForPage(mIsRtl ? childCount-2 : 1);
        // Perry: fix slide page exception: end
        // Perry: Optimizing sliding speed: start
        if (isLoopScroll()) {
            if (isXBeforeFirstPage()) {
                float factor = computeScrolledFactor(-2, width);
                mTransformer.transformPage(getPageAt(childCount - 1), factor);
                factor = computeScrolledFactor(0, width);
                mTransformer.transformPage(getPageAt(0), factor);
            } else if (isXAfterLastPage()) {
                float factor = computeScrolledFactor(childCount, width);
                mTransformer.transformPage(getPageAt(0), factor);
                factor = computeScrolledFactor(childCount-1, width);
                mTransformer.transformPage(getPageAt(childCount-1), factor);
            } else {
                for (int i = 0; i < childCount; i++) {
                    float factor = computeScrolledFactor(i, width);
                    mTransformer.transformPage(getChildAt(i), factor);
                }
            }
        } else {
            for (int i = 0; i < childCount; i++) {
    ...
    public void updatePageIndicatorState() {
        mPageIndicator.setMarkersCount(getChildCount());
    }
