...
public class CellLayout extends ViewGroup {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mFolderBackgrounds.size(); i++) {
            ...
            cellToPointWithMargin(bg.delegateCellX, bg.delegateCellY, mTempLocation);
        ...
        if (mFolderLeaveBehind.delegateCellX >= 0 && mFolderLeaveBehind.delegateCellY >= 0) {
            // Perry: Hotseat reorder function: start
            cellToPointWithMargin(mFolderLeaveBehind.delegateCellX,
    ...
    public boolean animateChildToPosition(final View child, int cellX, int cellY, int duration,
        ...
        if (clc.indexOfChild(child) != -1) {
            ...
            final int oldX = lp.x;
            final int oldY = lp.y;
            final int oldW = lp.width;
            final int oldH = lp.height;
            if (adjustOccupied) {
            ...
            if (mContainerType == HOTSEAT) {
                ...
                calculateCellSize(cellSize, mCountX, mCountY);
                lp.leftMargin = lp.rightMargin = Math.max(0, (cellSize[0] - getMinCellSize()) / 2);
                lp.setup(cellSize[0], cellSize[1], mShortcutsAndWidgets.invertLayoutHorizontally(), mCountX);
            } else {
                clc.setupLp(child);
            }
            ...
            final int newX = lp.x;
            final int newY = lp.y;
            final int newW = lp.width;
            final int newH = lp.height;
            ...
            va.addUpdateListener(new AnimatorUpdateListener() {
                ...
                public void onAnimationUpdate(ValueAnimator animation) {
                    ...
                    lp.width = (int) ((1 - r) * oldW + r * newW);
                    lp.height = (int) ((1 - r) * oldH + r * newH);
                    child.requestLayout();
                }
            });
            // Perry: Hotseat reorder function: end
    ...
    private void beginOrAdjustReorderPreviewAnimations(ItemConfiguration solution,
        ...
        if (mContainerType == HOTSEAT) return;
        int childCount = mShortcutsAndWidgets.getChildCount();
        for (int i = 0; i < childCount; i++) {
            View child = mShortcutsAndWidgets.getChildAt(i);
            if (child == dragView) continue;
    ...
    */
    ...
    protected void calculateCellSize(int[] cellSize, int countX, int countY) {
        int childWidthSize = getWidth() - getPaddingLeft() - getPaddingRight();
        int childHeightSize = getHeight() - getPaddingTop() - getPaddingBottom();
        cellSize[0] = DeviceProfile.calculateCellWidth(childWidthSize, countX);
        cellSize[1] = DeviceProfile.calculateCellHeight(childHeightSize, countY);
    }
        ...
        for (int x = 0; x < mCountX; x++) {
            for (int y = 0; y < mCountY; y++) {
                if (getChildAt(x, y) != null) {
                    mOccupied.markCells(x, y, 1, 1, true);
                } else {
                    mOccupied.markCells(x, y, 1, 1, false);
                }
            }
        }
    }
    ...
    public void setAlpha(float alpha) {
        if(PlatformFeatureFlags.SUPPORT_FOLDER_TAP_MODE && (getAlpha()==0f&&alpha == 1f)){
            AbstractFloatingView topOpenView = AbstractFloatingView.getTopOpenView(mLauncher);
            if(topOpenView instanceof FolderPagesContainer &&topOpenView.mIsOpen){
