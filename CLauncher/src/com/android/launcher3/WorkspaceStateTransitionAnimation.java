...
import static com.android.launcher3.LauncherState.EDIT_MODE_PANEL;
import static com.android.launcher3.LauncherState.HOTSEAT_ICONS;
import static com.android.launcher3.LauncherState.HOTSEAT_SEARCH_BOX;
import static com.android.launcher3.LauncherState.PAGE_INDICATOR;
...
public class WorkspaceStateTransitionAnimation {
    ...
    private void setWorkspaceProperty(LauncherState state, PropertySetter propertySetter,
        ...
        if (playAtomicComponent) {
            ...
            float pageIndicatorAlpha = (elements & PAGE_INDICATOR) != 0 ? 1 : 0;
            propertySetter.setViewAlpha(mLauncher.getWorkspace().getPageIndicator(),
                    pageIndicatorAlpha, fadeInterpolator);
            float editModePanelAlpha = (elements & EDIT_MODE_PANEL) != 0 ? 1 : 0;
            propertySetter.setViewAlpha(mLauncher.getEditModePanel(),
                    editModePanelAlpha, fadeInterpolator);
        ...
        // Perry: Add edit mode panel: start
        if (mLauncher.isInState(EDIT_MODE) || state == EDIT_MODE) {
            float pageIndicatorTransY = state.getPageIndicatorTranslationY(mLauncher);
            propertySetter.setFloat(mLauncher.getWorkspace().getPageIndicator(),
                    View.TRANSLATION_Y, pageIndicatorTransY, translationInterpolator);

            if ((elements & EDIT_MODE_PANEL) != 0) {
                EditModePanel panel = mLauncher.getEditModePanel();
                panel.setTranslationY(mLauncher.getWorkspace().getPageIndicator().getTranslationY() -
                        pageIndicatorTransY);
                propertySetter.setFloat(panel,
                        View.TRANSLATION_Y, 0, translationInterpolator);
            } else {
                pageIndicatorTransY = mLauncher.getWorkspace().getPageIndicator().getTranslationY();
                CellLayout hotseatLayout = mLauncher.getHotseat().getLayout();
                hotseatLayout.setTranslationY(-pageIndicatorTransY);
                propertySetter.setFloat(hotseatLayout,
                        View.TRANSLATION_Y, 0, translationInterpolator);
            }
        }
        // Perry: Add edit mode panel: end
