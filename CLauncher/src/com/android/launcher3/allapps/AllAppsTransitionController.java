...
public class AllAppsTransitionController implements StateHandler, OnDeviceProfileChangeListener {
    ...
    private void onProgressAnimationStart() {
        ...
        if (Float.compare(mProgress, 0f) == 0) {
            ...
            mAppsView.post(()->mAppsView.reset(false));
    ...
    public void setProgress(float progress) {
        ...
        if (mAppsView.getFloatingHeaderView().hasVisibleContent()) {
            AllAppsRecyclerView rc = mAppsView.getActiveRecyclerView();

            if (OVERVIEW.getVerticalProgress(mLauncher) <= progress) {
                rc.setProgress(0);
            } else if (mLauncher.isInState(ALL_APPS)) {
                rc.setProgress(Math.min(1 - progress, 1));
            } else if (mLauncher.isInState(OVERVIEW)) {
                rc.setProgress(Math.max(OVERVIEW.getVerticalProgress(mLauncher) - progress, 0));
            }

            if (mLauncher.isInState(LauncherState.OVERVIEW)) {
                rc.setOverScrollMode(View.OVER_SCROLL_NEVER);
            } else {
                rc.setOverScrollMode(View.OVER_SCROLL_ALWAYS);
            }
        }
    ...
    private void setAlphas(LauncherState toState, AnimationConfig config,
        ...
        setter.setViewAlpha(mAppsView.getSearchView(), hasHeader  || hasHeaderExtra ? 1 : 0, allAppsFade);
        setter.setViewAlpha(mAppsView.getContentView(), hasContent  || hasHeaderExtra ? 1 : 0, allAppsFade);
    ...
    public float getScrollRangeDelta() {
        return mScrollRangeDelta;
    }
    ...
    private void onProgressAnimationEnd() {
        if (Float.compare(mProgress, 1f) == 0) {
            ...
            // Perry: Implement frozen apps: start
            mAppsView.setState(AllAppsContainerView.State.NORMAL);
            // Perry: Implement frozen apps: end
