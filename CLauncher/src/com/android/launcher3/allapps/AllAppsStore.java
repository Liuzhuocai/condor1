...
public class AllAppsStore {
    ...
    private Predicate<AppInfo> mFilter = info-> !info.isFrozen();
    ...
    public AppInfo getApp(SearchKey key) {
        // Perry: Implement frozen apps: start
        AppInfo info = mComponentToAppMap.get(key);
        if (info!=null && mFilter.test(info)) {
            return info;
        }
    ...
    // Perry: Implement frozen apps: start
    public void setFilter(Predicate<AppInfo> filter) {
        mFilter = filter;
    }

    public Predicate<AppInfo> getFilter() {
        return mFilter;
    }
    // Perry: Implement frozen apps: end
