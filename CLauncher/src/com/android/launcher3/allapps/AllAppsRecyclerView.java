...
public class AllAppsRecyclerView extends BaseRecyclerView implements LogContainerProvider {
    ...
    private Runnable mDelayedSmoothScrollToRunnable;
    private Runnable mOnLayoutChangedRunnable;
    ...
    public AllAppsRecyclerView(Context context, AttributeSet attrs, int defStyleAttr,
        ...
        addOnLayoutChangeListener(new OnLayoutChangeListener() {
            ...
            public void onLayoutChange(View view, int i, int i1, int i2, int i3, int i4, int i5, int i6, int i7) {
                if (mOnLayoutChangedRunnable != null) {
                    mOnLayoutChangedRunnable.run();
                    mOnLayoutChangedRunnable = null;
                }
            }
        });
        // Perry: locate application function: end
    ...
    private void updatePoolSize() {
        ...
        pool.setMaxRecycledViews(AllAppsGridAdapter.VIEW_TYPE_SEARCH_RESULT_HEADER, SearchKey.TYPES_COUNT);
        pool.setMaxRecycledViews(AllAppsGridAdapter.VIEW_TYPE_SEARCH_APP, approxRows * mNumAppsPerRow);
        ...
        // Perry: Optimizing the scroll effect of search results: start
        mViewHeights.put(AllAppsGridAdapter.VIEW_TYPE_SEARCH_MARKET, Utilities.pxFromDp(48,
                getResources().getDisplayMetrics()));
        mViewHeights.put(AllAppsGridAdapter.VIEW_TYPE_SEARCH_APP, Utilities.pxFromDp(60,
                getResources().getDisplayMetrics()));
        // Perry: Optimizing the scroll effect of search results: end
    }
    ...
    public void onScrollStateChanged(int state) {
        ...
        if (state == SCROLL_STATE_IDLE) {
            if (mDelayedSmoothScrollToRunnable != null) {
                mDelayedSmoothScrollToRunnable.run();
                mDelayedSmoothScrollToRunnable = null;
            }
        }
    }

    public void setOnLayoutChangedRunnable(Runnable r) {
        if (mOnLayoutChangedRunnable != null) {
            mOnLayoutChangedRunnable.run();
        }

        mOnLayoutChangedRunnable = r;
    }

    public void smoothScrollToPosition(int position, Runnable r) {
        if (mDelayedSmoothScrollToRunnable != null) {
            mDelayedSmoothScrollToRunnable.run();
        }

        mDelayedSmoothScrollToRunnable = r;
        smoothScrollToPosition(position);
    }

    public int getAppPosition(final AppInfo app) {
        List<AlphabeticalAppsList.AdapterItem> items = mApps.getAdapterItems();
        for (AlphabeticalAppsList.AdapterItem item :items) {
            if (item.appInfo == null) {
                continue;
            }

            if (app.getTargetComponent().equals(item.appInfo.getTargetComponent())
                    && app.user.equals(item.appInfo.user)) {
    ...
    public void setProgress(float progress) {
        mApps.setOverviewProgress(progress);
        if (getAdapter() != null) {
