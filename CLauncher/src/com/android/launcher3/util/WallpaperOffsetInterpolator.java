...
public class WallpaperOffsetInterpolator extends BroadcastReceiver {
    ...
    private int mLockWallpaper = 0;
    public WallpaperOffsetInterpolator(Workspace workspace) {
        mWorkspace = workspace;
        mIsRtl = Utilities.isRtl(workspace.getResources());
        mHandler = new OffsetHandler(workspace.getContext());
        mLockWallpaper = Integer.valueOf(Switcher.WALLPAPER_LAYOUT_SWITCHER.getPersistence().value());
        Switcher.WALLPAPER_LAYOUT_SWITCHER.getPersistence().registerValueChangedListener(p -> mLockWallpaper = Integer.valueOf((String) p.value()));
    }
    ...
    private void wallpaperOffsetForScroll(int scroll, int numScrollingPages, final int[] out) {
        ...
        //liuzuo : add for lockwallpaper: begin
        if (mLockWallpaper == 1) {
            scroll = 0;
        } else if (mLockWallpaper == 2) {
            scroll = scrollRange/2;
        } else if (mLockWallpaper == 3) {
            scroll = scrollRange;
        }
        //liuzuo : add for lockwallpaper: end
        ...
        int pageIndex = mIsRtl ? leftPageIndex : rightPageIndex;
        if (adjustedScroll > scrollRange) {
            adjustedScroll = scrollRange - (adjustedScroll - scrollRange) * pageIndex;
        } else if (adjustedScroll < 0) {
            adjustedScroll = -adjustedScroll * pageIndex;
        }
