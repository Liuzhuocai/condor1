...
public class IconCache {
    ...
    public IconCache(Context context, InvariantDeviceProfile inv,IconProvider iconProvider) {
        mContext = context;
...
//        }
        ...
        if(mIconProvider.isThemeChanged()){
            this.clearIcons();
        }
        ...
        if (mIconProvider.isDefaultTheme()) {
            LiveIconsManager.obtain().init(context.getApplicationContext());
        }
    ...
    protected BitmapInfo makeDefaultIcon(UserHandle user) {
        try (LauncherIcons li = LauncherIcons.obtain(mContext)) {
            return li.createBadgedIconBitmap(
                    getFullResDefaultActivityIcon(), user, VERSION.SDK_INT,mContext);
        }
    }
        ...
        if (entry == null) {
            ...
            if(mIconProvider.isDefaultTheme()){
                li.createBadgedIconBitmap(
                        getFullResIcon(app), app.getUser(),
                        app.getApplicationInfo().targetSdkVersion,mContext).applyTo(entry);
            }else{
                entry.icon = li.createBadgedIconBitmapForExtral(getFullResIcon(app),app.getUser(), mContext);
            }
            li.recycle();
        ...
        if (entry == null || (entry.isLowResIcon && !useLowResIcon)) {
            ...
            if (!getEntryFromDB(cacheKey, entry, useLowResIcon) || DEBUG_IGNORE_CACHE) {
                ...
                if (info != null) {
                    ...
                    if(mIconProvider.isDefaultTheme()){
                        li.createBadgedIconBitmap(
                                getFullResIcon(info), info.getUser(),
                                info.getApplicationInfo().targetSdkVersion,mContext).applyTo(entry);
                    }else{
                        entry.icon = li.createBadgedIconBitmapForExtral(getFullResIcon(info),info.getUser(), mContext);
                    }
                    li.recycle();
                    //entry.icon = LauncherIcons.createBadgedIconBitmap(getFullResIcon(app), app.getUser(),
                    //        mContext,  app.getApplicationInfo().targetSdkVersion);
            ...
            if (!getEntryFromDB(cacheKey, entry, useLowResIcon)) {
                try {
                    ...
                    BitmapInfo iconInfo = li.createBadgedIconBitmap(
                            ...
                            mInstantAppResolver.isInstantApp(appInfo),mContext);
                    li.recycle();
    ...
    public void clearIcons() {
        flush();
        clear();
        mIconProvider.reInit();
    }

    public boolean isThemeChanged(){
        return mIconProvider.isThemeChanged();
    }
    public void flush() {
        synchronized (mCache) {
            mCache.clear();
    ...
    //antino:for theme:end
    //liuzuo:for theme config:begin
    public ThemeConfigBean getThemeConfigBean(){
        if (mIconProvider==null)
            return null;
        return mIconProvider.getThemeConfigBean();
    }
    public ThemeDescriptionBean getThemeDescriptionBean() {
        if (mIconProvider==null)
            return null;
        return mIconProvider.getThemeDescriptionBean();
    }
    public Bitmap getBitmapByName(String iconName) {
        if (mIconProvider==null)
            return null;
        return mIconProvider.getBitmapByName(iconName);
    }
    //liuzuo:for theme config:end

    // Perry: Implement Lock/Unlock task function: start
    public Drawable getIconFromCache(ComponentName componentName, UserHandle user) {
        CacheEntry entry = cacheLocked(componentName, new Provider<LauncherActivityInfo>() {
                    @Override
                    public LauncherActivityInfo get() {
                        return null;
                    }
        ...
        return new BitmapDrawable(mContext.getResources(),
                entry.icon == null ? getDefaultIcon(user).icon : entry.icon);
    }

    public Drawable getIconFromCache(ComponentName componentName, Intent intent, UserHandle user) {
        CacheEntry entry = cacheLocked(componentName, new ActivityInfoProvider(intent, user),
                user, true, false);
        return new BitmapDrawable(mContext.getResources(),
                entry.icon == null ? getDefaultIcon(user).icon : entry.icon);
    }
