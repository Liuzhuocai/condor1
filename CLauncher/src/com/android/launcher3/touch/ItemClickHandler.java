...
public class ItemClickHandler {
    ...
    private static void onClick(View v) {
        ...
        if (tag instanceof ShortcutInfo) {
            ...
            if(launcher.getImportMode()){
                Folder folder = Folder.getOpen(launcher);
                if(folder!=null){
                    folder.onImportModeClick(v, (ShortcutInfo) tag);
                }else {
                    launcher.onImportModeClick(v, (ShortcutInfo) tag);
                }
            }else {
            ...
            //liuzuo:dd for addIcon:begin
            if(v==launcher.getEditFolderIcon())return;
            //liuzuo:add for addIcon:end
            if (v instanceof FolderIcon) {
                onClickFolderIcon(v);
            }
        } else if (tag instanceof AppInfo) {
            if (launcher.isInFrozenState()) {
                FrozenAppsManager.obtain().toggleFrozen((AppInfo)tag);
                return;
            }
        ...
        //liuzuo:add the folderImportMode:begin
        else if (tag instanceof FolderPagedView.FolderAddInfo) {
            Folder folder = Folder.getOpen(launcher);
            if(folder!=null){
                folder.onImportAddIconClick();
            }
        }
        else if (v==launcher.getFolderImportButton()){
           launcher.onImportBtnClick();
        }
        //liuzuo:add the folderImportMode:end
    }
