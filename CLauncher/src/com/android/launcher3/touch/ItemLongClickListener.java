...
public class ItemLongClickListener {
    ...
    private static boolean onWorkspaceItemLongClick(View v) {
        ...
        if (!launcher.isInState(NORMAL) && !launcher.isInState(OVERVIEW) &&
                !launcher.isInState(EDIT_MODE)) return false;
    ...
    public static void beginDrag(View v, Launcher launcher, ItemInfo info,
        ...
        if (SCREEN_LOCKED.value()) {
            launcher.getWorkspace().showPopupContainer(v, dragOptions);
            ToastHelper.showMessage(launcher, R.string.msg_desktop_is_locked);
            return;
        }
