...
public class IconNormalizer {
    ...
    public static final float MAX_SQUARE_AREA_FACTOR = 375.0f / 576;
    // Ratio of icon visible area to full icon size for a circular shaped icon
    private static final float MAX_CIRCLE_AREA_FACTOR = 380.0f / 576;

    //antino:icon normalize:begin
    private static final float CIRCLE_AREA_BY_RECT = 0.93f;/*(float) Math.PI / 4*/;
    ...
    private final int totalRectSize;
    private final int totalCirleSize;
    //antino:icon normalize:end
    /** package private **/
    IconNormalizer(Context context) {
        ...
        //antino:icon normalize:begin
        radius = context.getResources().getDimensionPixelSize(R.dimen.icon_radius);
        totalSize = context.getResources().getDimensionPixelSize(R.dimen.total_size);
        totalRectSize = context.getResources().getDimensionPixelSize(R.dimen.total_rect_size);
        totalCirleSize = context.getResources().getDimensionPixelSize(R.dimen.total_cirle_size);
        //antino:icon normalize:end
    }
    ...
    //antino:icon normalize:begin
    public synchronized float getScale(@NonNull Drawable d, @Nullable RectF outBounds,
                                       @Nullable Path path, @Nullable boolean[] outMaskShape,Rect bounds,boolean[] isCirle) {
        if (Utilities.ATLEAST_OREO && d instanceof AdaptiveIconDrawable &&
                mAdaptiveIconScale != SCALE_NOT_INITIALIZED) {
            if (outBounds != null) {
                outBounds.set(mAdaptiveIconBounds);
        ...
        if(!(((float)mBounds.width()/mBounds.height())<1.03f&&((((float)mBounds.width()/mBounds.height()))>0.97f))){
            cirle = true;
        }
    ...
    public static Bitmap drawableToBitmap(Drawable drawable) {
        ...
        if(bitmap!=null&&bitmap.getWidth()!=drawable.getIntrinsicWidth()){
            Log.d("liuzuo44","drawable2bitmap");
            return PhotoUtils.drawable2bitmap(drawable);
        }
        return bitmap;
    }

    public Drawable clipByPathDrawable(Path path,Bitmap bitmap,Rect bounds,RectF iconBounds,Context context){
        if (!bitmap.isMutable()) {
            // 设置图片为背景为透明
        ...
        Bitmap bitmap1 = Bitmap.createBitmap(max,max, Bitmap.Config.ARGB_8888);
        BitmapShader mBitmapShader;
        mBitmapShader = new BitmapShader(bitmap1, Shader.TileMode.CLAMP, Shader.TileMode.CLAMP);
        ...
        float radiusScale =  (float) max/totalRectSize;
        canvas.drawRoundRect(new RectF(0,0,bitmap1.getWidth(),bitmap1.getHeight()),radius*radiusScale,radius*radiusScale,p);
        canvas.restore();
        ...
        float scaleBounds = 1.0f;
        if(mMaxSize<bitmap.getWidth()||mMaxSize < bitmap.getHeight()){
            scaleBounds = Math.max((float) bitmap.getWidth()/mMaxSize,(float)bitmap.getHeight()/mMaxSize);
        }

        Rect realBounds = new Rect();
        realBounds.left = (int)(bounds.left*scaleBounds);
        realBounds.right = (int)(bounds.right*scaleBounds);
        realBounds.top = (int)(bounds.top*scaleBounds);
        realBounds.bottom = (int)(bounds.bottom*scaleBounds);
        canvas.drawBitmap(bitmap,realBounds,new RectF(0,0,bitmap1.getWidth(),bitmap1.getHeight()),p);
        canvas.restore();
        p.setXfermode(null);
    ...
    public float getRealScale(Rect bounds ,boolean overOrea) {
        float realScale = 1.0f;
        int realWidth = bounds.width();
        int realHeight = bounds.height();
        int max  = Math.max(realHeight,realWidth);
        float deta = 0.1f;
        if(overOrea){
            deta = 0.06f;
        }
        if(max<totalRectSize){
            realScale = ((float) totalSize)/(totalRectSize+totalSize*deta);
        }else{
            realScale = ((float) totalSize)/(max+totalSize*deta);
        }
        return realScale;
    }
    //antino:icon normalize:end
