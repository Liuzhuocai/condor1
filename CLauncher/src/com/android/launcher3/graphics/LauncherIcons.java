...
public class LauncherIcons implements AutoCloseable {
    ...
    //antino:icon normalize:begin
    private static boolean useSmallBounds;
    private static final ArrayList<String> useBigBoundsList = new ArrayList<String>();
    //antino:icon normalize:end
    /**
    ...
    static {
        //antino:icon normalize:begin
        useBigBoundsList.add("Allure M3");
        useSmallBounds = useBigBoundsList.contains(Build.MODEL);
        //antino:icon normalize:end
    }
        ...
        try {
            ...
            if (resources != null) {
                ...
                return createBadgedIconBitmap(
                        ...
                        0 /* do not apply legacy treatment */,mContext);
            }
        } catch (Exception e) {
            // Icon not found.
        }
     ...
     */
    public BitmapInfo createBadgedIconBitmap(Drawable icon, UserHandle user, int iconAppTargetSdk,Context context) {
        return createBadgedIconBitmap(icon, user, iconAppTargetSdk, false, context);
    }

    public BitmapInfo createBadgedIconBitmap(Drawable icon, UserHandle user, int iconAppTargetSdk,
            boolean isInstantApp,Context context) {
        return createBadgedIconBitmap(icon, user, iconAppTargetSdk, isInstantApp, null,context);
    }
     ...
     */
    public BitmapInfo createBadgedIconBitmap(Drawable icon, UserHandle user, int iconAppTargetSdk,
            boolean isInstantApp, float [] scale,Context context) {
        if (scale == null) {
            scale = new float[1];
        }
        icon = normalizeAndWrapToAdaptiveIcon(icon, iconAppTargetSdk, null, scale,context);
     ...
     */
    public Bitmap createScaledBitmapWithoutShadow(Drawable icon, int iconAppTargetSdk, Context context) {
        RectF iconBounds = new RectF();
        float[] scale = new float[1];
        icon = normalizeAndWrapToAdaptiveIcon(icon, iconAppTargetSdk, iconBounds, scale, context);
    ...
    private Drawable normalizeAndWrapToAdaptiveIcon(Drawable icon, int iconAppTargetSdk,
                                                    RectF outIconBounds, float[] outScale, Context context) {

        //antino:icon normalize:begin
        boolean[] isCircle = new boolean[1];
        ...
        if ((Utilities.ATLEAST_OREO && iconAppTargetSdk >= Build.VERSION_CODES.O) ||
            ...
            scale = getNormalizer().getScale(icon, outIconBounds, dr.getIconMask(), outShape,bounds,isCircle);
            //antino:icon normalize:end


                //antino:icon normalize:begin
                if(isCircle[0]&&(icon instanceof BitmapDrawable)){
                    float realScale;
                    if(useSmallBounds){
                        realScale = ShadowGenerator.getScaleForBounds(iconBounds);
                    }else{
                        realScale = getNormalizer().getRealScale(bounds,false);
                    }
                    Drawable wrappedIcon = wrapToAdaptiveIconDrawable(dr, icon, realScale);
                    if (wrappedIcon != icon) {
                        icon = wrappedIcon;
                        scale = getNormalizer().getScale(icon, null, null, null);
                    }
                }else if(icon instanceof BitmapDrawable){
                    icon = getNormalizer().clipByPathDrawable(dr.getIconMask(),getNormalizer().drawableToBitmap(icon),bounds,iconBounds,context);
                    scale = getNormalizer().getScale(icon, null, null, null);
                }else{
                    Drawable wrappedIcon = wrapToAdaptiveIconDrawable(dr, icon, scale);
                    if (wrappedIcon != icon) {
                        icon = wrappedIcon;
                        scale = getNormalizer().getScale(icon, null, null, null);
                    }
            ...
            if((icon instanceof BitmapDrawable)){
                scale = getNormalizer().getScale(icon, iconBounds, null, null, bounds, isCircle);
                if (isCircle[0]) {
                    float realScale;
                    if(useSmallBounds){
                        realScale = ShadowGenerator.getScaleForBounds(iconBounds);
                    ...
                    icon = wrapToAdaptiveIconDrawable(dr, icon, realScale);
                    scale = getNormalizer().getScale(icon, null, null, null);
                }else{
                    icon = getNormalizer().clipByPathDrawable(null,getNormalizer().drawableToBitmap(icon),bounds,iconBounds,context);
                    scale = getNormalizer().getScale(icon, null, null, null);
                }
            }else{
                scale = getNormalizer().getScale(icon, null, null, null);
    ...
    public BitmapInfo createShortcutIcon(ShortcutInfoCompat shortcutInfo,
        ...
        if (unbadgedDrawable != null) {
            unbadgedBitmap = createScaledBitmapWithoutShadow(unbadgedDrawable, 0, mContext);
        } else {
    ...
    public  Bitmap createBadgedIconBitmapForExtral(
        ...
        float scale  = normalizer.getScale(icon, null, null, null);
    ...
    static Drawable wrapToAdaptiveIconDrawable(AdaptiveIconDrawable dr, Drawable icon, float scale) {
