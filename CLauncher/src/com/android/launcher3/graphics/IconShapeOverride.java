...
//        }
    ...
    private static String getAppliedValue(Context context) {
        ...
        if (DesktopModeHelper.isDefaultMode()) {
            return context.getString(R.string.default_icon_shape_override_paths_values);
        }
        // Perry: Icon shape override for different desktop modes: end

        return getDevicePrefs(context).getString(KEY_PREFERENCE,
                //liuzuo:default icon style:start
                context.getString(R.string.default_icon_shape_override_paths_values));
                //liuzuo:default icon style:end
    }
    ...
    private static class PreferenceChangeHandler implements OnPreferenceChangeListener {
        ...
        public boolean onPreferenceChange(Preference preference, Object o) {
            ...
            if (!getAppliedValue(mContext).equals(newValue)) {
                ...
                if (model != null) {
                    model.schedule(()-> {
                        getDevicePrefs(mContext).edit().putString(KEY_PREFERENCE, newValue).apply();
                        apply(mContext);
                        Utils.resetAdaptiveIconMask();
                        app.clearIcons();
                        LauncherIcons.obtain(mContext).clear();
