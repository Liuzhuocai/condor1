...
public class LauncherStateManager {
    ...
    public void goToStateFrom(LauncherState state, LauncherState from) {
        if (from == mState) {
            goToState(state);
        }
    }
    ...
    // Perry: Add edit mode panel: start
    public void goToStateFrom(LauncherState state, LauncherState from,
                              long delay, Runnable onCompleteRunnable) {
        if (from == mState) {
            goToState(state, delay, onCompleteRunnable);
        }
    }
    // Perry: Add edit mode panel: end
    ...
    // Perry: Add edit mode panel: start
    public void goToStateFrom(LauncherState state, LauncherState from, long delay) {
        if (from == mState) {
            goToState(state, delay);
        }
    }
    // Perry: Add edit mode panel: end
    ...
    private void goToState(LauncherState state, boolean animated, long delay,
        ...
        if (mLauncher.isInState(state)
                || mState == EDIT_MODE && state == SPRING_LOADED) {
            if (mConfig.mCurrentAnimation == null) {
    ...
    private void onStateTransitionEnd(LauncherState state) {
        ...
        if (!state.requestFocus(mLauncher)) {
