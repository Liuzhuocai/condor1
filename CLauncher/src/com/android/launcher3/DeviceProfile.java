...
public class DeviceProfile {
    ...
    //liuzuo : for overview Mode : start
    public float workspaceOverviewModeShrinkFactor;
    //liuzuo : for overview Mode : end
    ...
    //liuzuo:change folder icon:begin
    public int folderBackgroundOffset;
    public int folderIconPreviewPadding;
    //liuzuo:change folder icon:end
    ...
    public int folderPagePaddingLeft;
    public int folderPagePaddingTop;
    ...
    // Perry: Optimizing recents in multi window mode: start
    public final int allAppsSearchBarFieldHeightPx;
    // Perry: Optimizing recents in multi window mode: end
    ...
    private final int editModeMinIconZoneHeightPx;
    private final int editModeMaxIconZoneHeightPx;
    public final int editModeBarItemWidthPx;
    public final int editModeBarSpacerWidthPx;
    private final float editModeIconZoneRatio;
    ...
    public DeviceProfile(Context context, InvariantDeviceProfile inv,
        ...
        folderPagePaddingLeft = edgeMarginPx;
        folderPagePaddingTop  = res.getDimensionPixelSize(R.dimen.folder_tab_margin_top) +
                res.getDimensionPixelSize(R.dimen.folder_tab_height) +
                res.getDimensionPixelSize(R.dimen.folder_pager_margin_top);
        ...
        // Perry: Optimizing recents in multi window mode: start
        allAppsSearchBarFieldHeightPx = res.getDimensionPixelSize(R.dimen.all_apps_search_bar_field_height)
            - getAllAppsSearchBarTranslationY(context);
        // Perry: Optimizing recents in multi window mode: end

        // Perry: Add edit mode panel: start
        editModeMinIconZoneHeightPx =
                res.getDimensionPixelSize(R.dimen.dynamic_grid_editmode_min_icon_zone_height);
        editModeMaxIconZoneHeightPx =
                res.getDimensionPixelSize(R.dimen.dynamic_grid_editmode_max_icon_zone_height);
        editModeBarItemWidthPx =
                res.getDimensionPixelSize(R.dimen.dynamic_grid_editmode_bar_item_width);
        editModeBarSpacerWidthPx =
                res.getDimensionPixelSize(R.dimen.dynamic_grid_editmode_bar_spacer_width);
        editModeIconZoneRatio =
                res.getInteger(R.integer.config_dynamic_grid_editmode_icon_zone_percentage) / 100f;
        // Perry: Add edit mode panel: end
        ...
        mBadgeRenderer = new BadgeRenderer(iconSizePx,context.getResources());
    }
    ...
    private void updateIconSize(float scale, Resources res, DisplayMetrics dm) {
        ...
        // Hotseat
        if (isVerticalLayout) {
            hotseatBarSizePx = hotseatCellHeightPx + hotseatBarSidePaddingStartPx
                    + hotseatBarSidePaddingEndPx;
        } else {
            hotseatBarSizePx = hotseatCellHeightPx + hotseatBarTopPaddingPx + hotseatBarBottomPaddingPx;
        }
        // Perry: Adjust hotseat bar size: end
        ...
        if (!isVerticalLayout) {
            ...
            workspaceOverviewModeShrinkFactor =
    ...
    private void updateAvailableFolderCellDimensions(DisplayMetrics dm, Resources res) {
        ...
        if(PlatformFeatureFlags.SUPPORT_FOLDER_TAP_MODE){
             maxHeight = availableHeightPx - folderPagePaddingTop;
        }
        ...
        if(PlatformFeatureFlags.SUPPORT_FOLDER_TAP_MODE){
            maxWidth = availableWidthPx - folderPagePaddingLeft * 2;
        }
    ...
    private void updateFolderCellSize(float scale, DisplayMetrics dm, Resources res) {
        ...
        //liuzuo :modify landscape folder width:start
        if (isVerticalBarLayout()) {
            folderCellWidthPx = availableWidthPx / inv.numFolderColumns;
        }
        //liuzuo :modify landscape folder width:end
    ...
    public Rect getHotseatLayoutPadding() {
        if (isVerticalBarLayout()) {
            ...
            // Perry: Optimizing hotseat padding: start
            float hotseatCellWidth = (float) widthPx / inv.numColumns;
            // Perry: Optimizing hotseat padding: end
    ...
    // Perry: Add edit mode panel: start
    public int getEditModeButtonBarHeight() {
        int zoneHeight = (int) (editModeIconZoneRatio * availableHeightPx);
        zoneHeight = Math.min(editModeMaxIconZoneHeightPx,
                Math.max(editModeMinIconZoneHeightPx, zoneHeight));
        return zoneHeight;
    }
    // Perry: Add edit mode panel: end

    // Perry: Optimizing recents in multi window mode: start
    public int getAllAppsSearchBarTranslationY(Context c) {
        if (isVerticalBarLayout()) {
            return Utils.dp2px(c, 25);
        }

        return Utils.dp2px(c, 32);
    }
    // Perry: Optimizing recents in multi window mode: end
}
